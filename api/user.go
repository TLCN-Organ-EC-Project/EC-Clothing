...
func (server *Server) createUser(ctx *gin.Context) {
	...
	province, err := server.store.GetProvince(ctx, req.Province)
	if err != nil {
		ctx.JSON(http.StatusInternalServerError, errorResponse(err))
		return
	}
...
func (server *Server) listUser(ctx *gin.Context) {
	...
	for _, user := range users {
		listUser = append(listUser, newUserResponse(user))
	}
...
func (server *Server) updateUser(ctx *gin.Context) {
	...
	province, err := server.store.GetProvince(ctx, reqUpdate.Province)
	if err != nil {
		ctx.JSON(http.StatusInternalServerError, errorResponse(err))
		return
	}
...
func (server *Server) sendResetPasswordToken(ctx *gin.Context) {
	...
	subject := "EC-Clothing verification code:"
	content := fmt.Sprintf(`
		<html>
	  	<head>
	  ...
	  <body>
	    <div class="container">
	      <h1>EC-Clothing</h1>
	      <p>
		  		Chào mừng đến với Shop thời trang của chúng tôi! Chúng tôi cung cấp các trang
		  		phục sành điệu và thời trang nhất. Chúng tôi hy vọng sẽ
		  		mang đến cho bạn một trải nghiệm mua sắm tuyệt vời.
	      </p>
	      <p>
...
// @Failure 404 {string} error
// @Failure 500 {string} error
// @Router /api/users/{username}/change [post]
