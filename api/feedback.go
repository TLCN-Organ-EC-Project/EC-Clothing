...
type getUserAndProduct struct {
	UserComment      string `uri:"username" binding:"required,alphanum"`
	ProductCommented int64  `uri:"product_commented" binding:"required"`
}
...
// @Param product_commented path string true "ProductCommented"
// @Security bearerAuth
...
// @Router /api/users/{username}/feedbacks/{product_commented} [post]
func (server *Server) createFeedback(ctx *gin.Context) {
	var reqCreate createFeedbackRequest
	var reqGet getUserAndProduct
	...
	product, err := server.store.GetProduct(ctx, reqGet.ProductCommented)
	if err != nil {
		if err == sql.ErrNoRows {
			ctx.JSON(http.StatusNotFound, errorResponse(err))
			return
...
// @Failure 400 {string} error
// @Failure 500 {string} error
// @Router /api/products/{id}/feedbacks [get]
func (server *Server) listFeedbackByID(ctx *gin.Context) {
	...
	arg := db.ListFeedbacksParams{
		ProductCommented: reqProduct.ID,
		Limit:            req.PageSize,
...
type updateFeedbackRequest struct {
	ID               int64  `uri:"id" binding:"required,min=1"`
	UserComment      string `uri:"username" binding:"required,alphanum"`
	ProductCommented int64  `uri:"product_commented" binding:"required,min=1"`
}
...
// @Failure 404 {string} error
// @Failure 500 {string} error
// @Router /api/users/{username}/feedbacks/{product_commented}/{id} [put]
func (server *Server) updateFeedback(ctx *gin.Context) {
	...
	if feedback.ProductCommented != product.ID {
		err := errors.New("this feedback doesn't belong to this product ")
		ctx.JSON(http.StatusBadRequest, errorResponse(err))
		return
	}
...
// @Failure 404 {string} error
// @Failure 500 {string} error
// @Router /api/users/{username}/feedbacks/{product_commented}/{id} [delete]
...
// @Failure 404 {string} error
// @Failure 500 {string} error
// @Router /api/admin/users/{username}/feedbacks/{product_commented}/{id} [delete]
